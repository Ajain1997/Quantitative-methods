# -*- coding: utf-8 -*-
"""
Created on Tue Feb 20 10:50:21 2024
...
u,s,vh = np.linalg.svd(std_data.loc[in_sample_period,:])

cumul_pcs = np.where((np.cumsum(s**2)/np.sum(s**2))>.5)[0][0]+1

pcs = pd.DataFrame(index = std_data.index,data = std_data@(vh[:cumul_pcs,:].T))
...
rets_df = sm.OLS(endog=rets_data.loc[in_sample_period],exog = sm.add_constant(pcs.shift(1).loc[in_sample_period]), missing= 'drop').fit()

pcs_coef = pd.DataFrame(index = rets_df.params.index ,columns = rets.columns, data = rets_df.params.values)
resid_direct = pd.DataFrame(index = rets_df.resid.index, columns = rets.columns, data= rets_df.resid.values)

mu_direct = pd.DataFrame(index = out_of_sample_period, columns = rets.columns, data = sm.add_constant(pcs.shift(1).loc[out_of_sample_period]) @ pcs_coef)
...
fac_ols = sm.OLS(endog = data[['Mkt-RF','SMB','HML']].loc[in_sample_period],exog = sm.add_constant(pcs.shift(1).loc[in_sample_period]),missing='drop').fit()
...
out_of_sample_forecast = pd.DataFrame(index = out_of_sample_period, columns = ['Mkt-RF','SMB','HML'], data = sm.add_constant(pcs.shift(1).loc[out_of_sample_period]) @ pred)
...
plt.legend()
print("When considering how to predict investment returns, it's crucial to factor in trading costs. One method involves directly assessing the expected returns of each investment and its allocation within the portfolio. This approach typically requires less frequent trading, resulting in lower transaction expenses. However, it may not fully exploit the benefits of diversification or identify patterns across different types of investments. Alternatively, employing models that analyze broader market trends, such as correlations between certain types of investments, may necessitate more frequent portfolio adjustments and incur higher trading fees. Nonetheless, these models can offer protection during challenging market conditions, as demonstrated during events like the 2008 financial crisis. When deciding between these approaches, it's important to consider both risk tolerance and investment objectives.")
